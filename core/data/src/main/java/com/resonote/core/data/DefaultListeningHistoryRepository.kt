...
internal class DefaultListeningHistoryRepository internal constructor(
    ...
    override fun observeDeviceHistory(): Flow<List<DeviceHistoryItem>> =
        deviceHistory.observeAll().map { rows -> rows.map { it.asExternalModel() } }

    override suspend fun recordDevicePlayback(record: DeviceHistoryRecord): Boolean =
        mutate { deviceHistory.record(record.toEntity(now())) }

    override suspend fun deleteDeviceHistory(record: DeviceHistoryRecord): Boolean =
        mutate { check(deviceHistory.delete(record.toEntity(0).source, record.mediaId) == 1) }
