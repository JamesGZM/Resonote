...
internal class DefaultSearchRepository @Inject constructor(
    ...
    override suspend fun searchSongs(keywords: String, page: Int, pageSize: Int) =
        searchPage(keywords, page, pageSize) {
            val value = searchNetwork.searchSongs(it, page, pageSize)
            SearchPage(
                value.items.map { song -> song.toOnlineSong() },
                page,
                value.total,
                hasMore(value.total, page, pageSize, value.items.size),
            )
        }
    ...
    override suspend fun searchMvs(keywords: String, page: Int, pageSize: Int) = searchPage(keywords, page, pageSize) {
        ...
        SearchPage(
            value.items.map { item ->
                SearchMv(item.hash, item.name, item.singer, item.coverUrl?.size(480), item.durationMillis)
            },
            page,
            value.total,
            value.hasMore,
    ...
    override suspend fun searchComplex(keywords: String) = loadCollection(riskChallenges) {
        ...
        ComplexSearchResult(
            ...
            value.albums.map {
                SearchAlbum(it.id, it.name, it.artist, it.coverUrl?.size(480), it.songCount, it.publishDate)
            },
            value.albumsTotal,
            value.playlists.map {
                SearchPlaylist(it.id, it.name, it.creator, it.coverUrl?.size(480), it.songCount, it.playCount)
            },
            value.playlistsTotal,
            value.mvs.map {
                SearchMv(it.hash, it.name, it.singer, it.coverUrl?.size(480), it.durationMillis)
            },
            value.mvsTotal,
        )
    }
    ...
    private fun hasMore(total: Int, page: Int, pageSize: Int, received: Int) = if (total >
        0
    ) {
        page.toLong() * pageSize < total
    } else {
        received >= pageSize
    }
