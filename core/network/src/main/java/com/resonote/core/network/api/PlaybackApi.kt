...
internal interface PlaybackApi {
    ...
    suspend fun songSource(
        ...
        @Query("IsFreePart") isFreePart: String,
    ...
    suspend fun cloudSongUrl(
        @Query("hash") hash: String,
        @Query("ssa_flag") ssaFlag: String = "is_fromtrack",
        @Query("version") version: Int = 20102,
        @Query("ssl") ssl: Int = 0,
        @Query("album_audio_id") albumAudioId: String,
        @Query("pid") pid: Int = 20026,
        @Query("audio_id") audioId: Int = 0,
        @Query("kv_id") kvId: Int = 2,
        @Query("key") key: String,
        @Query("bucket") bucket: String = "musicclound",
