...
- The artwork page uses a square cover up to `380dp` with `20dp` horizontal margins, rounded corners, a dark elevation shadow, and a larger low-opacity blurred artwork copy beneath it for the diffuse colored glow visible in the approved design. The page background independently uses a more strongly scaled and blurred artwork layer under a contrast scrim.
