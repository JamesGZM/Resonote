...
- 首页与搜索页话筒统一进入类型安全 `RecognitionNavKey`。用户在独立页面明确开始后才请求 `RECORD_AUDIO`；普通拒绝可再次请求，永久拒绝提供系统应用设置入口，返回前台后重新读取真实权限状态。
- `RecognitionRecorder` 是 Feature 内可替换的平台边界；生产实现用 `AudioRecord` 采集 8 kHz、单声道、16 位 PCM，最长 10 秒且支持提前停止。少于 1 秒的 PCM 不调用 Repository；页面离开、取消和配置重建会终止活动采集。
- 合格 PCM 只驻留内存并提交给 `RecognitionRepository` 的 `API-RECOGNITION-001`，请求完成、取消或失败后立即覆零；不写临时文件、不进入媒体库/历史，也不增加分析或网络调用。
- 无匹配是独立成功结果，不映射为网络错误；网络、认证、风控、服务和协议失败保留 `ContentFailure`。失败后的“重新听一次”重新采集，不重用或自动重放已经清除的音频。
- 真正开始采集前 App 暂停 Resonote 音乐但保留 Queue 与当前项，识别结束不自动恢复。匹配结果按 Repository 顺序展示，用户可明确播放候选，或携带歌曲名与艺人进入已有搜索页。
