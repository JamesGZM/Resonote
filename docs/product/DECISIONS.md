...
| 2026-08-27 | P-109 | 歌词设置页不使用额外的分组标题或分组容器，所有设置项以普通平铺列表和分隔线表达；其中“桌面歌词”是一个带尾部箭头的子页面入口。桌面歌词子页面同样不使用“外观”或“控制器行为”等分组样式 | 用户明确要求歌词设置与桌面歌词设置去分组化，并保留桌面歌词子页面层级 | Lyrics Settings、Desktop Lyrics Settings、Navigation 和 Design System | 已确认；现行合同 |
